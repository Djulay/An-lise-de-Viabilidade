    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-6 md:ml-64">
            ...
            <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
                <h1 class="text-2xl font-bold text-gray-800 mb-6">Editar Curso: {{ $curso->nome }}</h1>
                <form action="{{ route('admin.cursos.update', $curso->id) }}" method="POST"
                    ...
                    <div>
                        ...
                        @if ($curso->imagem)
                            <img src="{{ asset('storage/' . $curso->imagem) }}" alt="{{ $curso->nome }}"
                                class="w-32 h-32 object-cover mb-2 rounded-md">
                        @endif
                    ...
                    <div>
                        ...
                        <input type="text" name="duracao" id="duracao"
                            value="{{ old('duracao', $curso->duracao) }}" placeholder="Ex: 6 meses"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('duracao')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="custo" id="custo"
                            value="{{ old('custo', $curso->custo) }}" placeholder="Ex: 50000" step="0.01"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('custo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="requisito" id="requisito"
                            value="{{ old('requisito', $curso->requisito) }}" placeholder="Ex: Ensino médio completo"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('requisito')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="modalidade" id="modalidade"
                            ...
                            <option value="Presencial"
                                {{ old('modalidade', $curso->modalidade) === 'Presencial' ? 'selected' : '' }}>
                                Presencial</option>
                            <option value="Hibrida"
                                {{ old('modalidade', $curso->modalidade) === 'Hibrida' ? 'selected' : '' }}>Híbrida
                            </option>
                        </select>
                        @error('modalidade')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="local" id="local"
                            value="{{ old('local', $curso->local) }}" placeholder="Ex: Luanda"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('local')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="ofertas" id="ofertas"
                            value="{{ old('ofertas', $curso->ofertas) }}" placeholder="Ex: Certificado incluso"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('ofertas')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select name="slug" id="slug"
                            ...
                            <option value="não destacar" {{ $curso->slug === 'não destacar' ? 'selected' : '' }}>Não
                                destacar</option>
                            <option value="destacar" {{ $curso->slug === 'destacar' ? 'selected' : '' }}>Destacar
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('admin.cursos.index') }}"
                            class="bg-gray-500 text-white px-6 py-2 rounded-md shadow-sm hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition duration-200">Cancelar</a>
                        <button type="submit"
